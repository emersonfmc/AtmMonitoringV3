...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('atm_client_banks_transaction_approvals', function (Blueprint $table) {
            ...
            $table->enum('status', ['Completed', 'Pending', 'Stand By', 'Cancelled', 'Other Rider'])->nullable();
            $table->enum('type', ['Received', 'Released'])->nullable();

            $table->foreign('employee_id', 'fk_employee_id')->references('employee_id')->on('users')->onDelete('cascade')->onUpdate('cascade');
            ...
            $table->foreign('user_groups_id', 'fk_user_groups_id')->references('id')->on('data_user_groups')->onDelete('cascade')->onUpdate('cascade');
